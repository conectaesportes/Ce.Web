import arenas from "../../data/arenas";
import {
    BrowserRouter as Router,
    Route,
    Routes,
    useParams,
} from "react-router-dom";
import AreaCard from "./components/AreaCard";
import Header from "../../pages/Components/Header";
import { useState } from "react";

import "./Arena.scss";
import AgendamentoArea from "./components/AgendamentoArea";

const Arena = () => {
    const [pagina, setPagina] = useState("lista");
    const [quadraSelecionada, setQuadraSelecionada] = useState(null);

    const { slug } = useParams(); // Recupera o slug da URL
    const pageData = arenas.find((item) => item.slug === slug); // Busca dados com base no slug

    if (!pageData) {
        return <h1>Página não encontrada</h1>;
    }

    return (
        <div className="container-arena page">
            <Header></Header>
            {pagina === "lista" ? (
                <>
                    <div className="header">
                        <div className="container-logo">
                            <img src={pageData.logo}></img>
                        </div>
                        <div className="container-info">
                            <h3 className="title">{pageData.nome}</h3>
                            <p>
                                {pageData.endereco.rua},{" "}
                                {pageData.endereco.numero} -{" "}
                                {pageData.endereco.bairro}
                            </p>
                        </div>
                    </div>
                    <div className="main-content">
                        <h3>Quadras Livres</h3>

                        <div className="list-areas">
                            {pageData.areasDeJogo.map((area) => {
                                return (
                                    <div
                                        key={area.nome}
                                        onClick={() => {
                                            setQuadraSelecionada(area);
                                            setPagina("agendamento");
                                        }}
                                    >
                                        <AreaCard
                                            key={area.nome}
                                            quadra={area}
                                            onClick={setPagina}
                                        ></AreaCard>
                                    </div>
                                );
                            })}
                        </div>
                    </div>
                </>
            ) : <AgendamentoArea quadra={quadraSelecionada}></AgendamentoArea>}
        </div>
    );
};

export default Arena;
